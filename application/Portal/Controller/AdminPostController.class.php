...
class AdminPostController extends AdminbaseController {
	...
	function _initialize() {
		...
		$this->shop = M("shop");
		$this->shop_relationships= M("shop_relationships");
	}
...
public function indexa(){
		$this->_listsa(array("post_status"=>array('neq',3)));
		$this->_getTree();
		$this->display();
	}
	//热点管理
	public function indexb(){
		$this->_listsb(array("post_status"=>array('neq',3)));
		$this->_getTree();
		$this->display();
	}
	//北京管理
	public function indexc(){
		$this->_listsc(array("post_status"=>array('neq',3)));
		$this->_getTree();
		$this->display();
	}
	//娱乐管理
	public function indexe(){
		$this->_listse(array("post_status"=>array('neq',3)));
		$this->_getTree();
		$this->display();
	}
	//国际期货
	...
	public function indexg(){
 	...
 	$shop=M("shop");
 	$shop_relationships=M("shop_relationships");
 	$result=$shop_relationships->where(array("term_id"=>11))->select();
 	foreach ($result as $key) {
 	 	 $wen=$shop->where(array('id'=>$key["object_id"]))->select();
	...
	public function indexh(){
 	...
 	$shop=M("shop");
 	$shop_relationships=M("shop_relationships");
 	$result=$shop_relationships->where(array("term_id"=>12))->select();
 	foreach ($result as $key) {
 	 	 $wen=$shop->where(array('id'=>$key["object_id"]))->select();
	...
	public function indexj(){
 	...
 	$shop=M("shop");
 	$shop_relationships=M("shop_relationships");
 	$result=$shop_relationships->where(array("term_id"=>13))->select();
 	foreach ($result as $key) {
 	 	 $wen=$shop->where(array('id'=>$key["object_id"]))->select();
	...
	public function indexk(){
 	...
 	$shop=M("shop");
 	$shop_relationships=M("shop_relationships");
 	$result=$shop_relationships->where(array("term_id"=>14))->select();
 	foreach ($result as $key) {
 	 	 $wen=$shop->where(array('id'=>$key["object_id"]))->select();
	...
	public function indexl(){
 	...
 	$shop=M("shop");
 	$shop_relationships=M("shop_relationships");
 	$result=$shop_relationships->where(array("term_id"=>15))->select();
 	foreach ($result as $key) {
 	 	 $wen=$shop->where(array('id'=>$key["object_id"]))->select();
	...
	public function indexm(){
 	...
 	$shop=M("shop");
 	$shop_relationships=M("shop_relationships");
 	$result=$shop_relationships->where(array("term_id"=>17))->select();
 	foreach ($result as $key) {
 	 	 $wen=$shop->where(array('id'=>$key["object_id"]))->select();
	...
	public function indexn(){
 	...
 	$shop=M("shop");
 	$shop_relationships=M("shop_relationships");
 	$result=$shop_relationships->where(array("term_id"=>16))->select();
 	foreach ($result as $key) {
 	 	 $wen=$shop->where(array('id'=>$key["object_id"]))->select();
	...
	public function addf(){
		//$termll=I("term");
		//$terms = $this->terms_model->order(array("listorder"=>"asc"))->select();
			$term_id = I("get.term",10,'intval');
		//$this->_getTermTreef();
		//$term=$this->terms_model->where(array('term_id'=>$term_id))->find();
		//$this->assign("term",$term);
	//	$this->assign("termll",$termll);
		$this->assign("terms",$term_id);
		$this->display();
	}
	public function addg(){
		$term_id = I("get.term",11,'intval');
		$this->assign("terms",$term_id);
		$this->display();
	}
	public function addh(){
	$term_id = I("get.term",12,'intval');
		$this->assign("terms",$term_id);
		$this->display();
	}
	public function addj(){
	$term_id = I("get.term",13,'intval');
		$this->assign("terms",$term_id);
		$this->display();
	}
	public function addk(){
	$term_id = I("get.term",14,'intval');
		$this->assign("terms",$term_id);
		$this->display();
	}
	public function addl(){
	$term_id = I("get.term",15,'intval');
		$this->assign("terms",$term_id);
		$this->display();
	}
	public function addm(){
	$term_id = I("get.term",17,'intval');
		$this->assign("terms",$term_id);
		$this->display();
	}
	public function addn(){
	$term_id = I("get.term",16,'intval');
		$this->assign("terms",$term_id);
		$this->display();
	}
	...
	public function add_post(){
		if (IS_POST) {
			...
			//$this->error(htmlspecialchars_decode($article['post_content']));
	...
	public function add_postf(){
		...
		if (IS_POST) {
			...
			$_POST['post']['shijian']=date("Y-m-d H:i:s",time());
			$_POST['post']['faburenid']=0;
			...
			if ($result) {
				foreach ($_POST['term'] as $mterm_id){
					$shop_relationships->add(array("term_id"=>intval($mterm_id),"object_id"=>$result));
				}
	...
	public function edit_post(){
		...
		if (IS_POST) {
			...
			// $shop_relationships->where(array("object_id"=>$post_id,"term_id"=>array("not in",implode(",", $_POST['term']))))->delete();

			// foreach ($_POST['term'] as $mterm_id){
			// 	$find_term_relationship=$shop_relationships->where(array("object_id"=>$post_id,"term_id"=>$mterm_id))->count();
			// 	if(empty($find_term_relationship)){
			// 		$shop_relationships->add(array("term_id"=>intval($mterm_id),"object_id"=>$post_id));
			// 	}else{
			// 		$shop_relationships->where(array("object_id"=>$post_id,"term_id"=>$mterm_id))->save(array("status"=>1));
			// 	}
			// }
			...
			$_POST['smeta']['thumb'] = sp_asset_relative_url($_POST['smeta']['thumb']);
	unset($_POST['post']['post_author']);
			$_POST['shijian']=date("Y-m-d H:i:s",time());
			 $_POST['faburenid']=0;
			...
			$article['xuqiu']=htmlspecialchars_decode($article['xuqiu']);
// $this->error($id);
			$result=$shop->where(array('id' =>$id ))->save($article);
	...
	private function _listsa($where=array()){
		//sp_sql_posts_bycatid($cid,$tag,$where);
	...
	private function _listsb($where=array()){
		//sp_sql_posts_bycatid($cid,$tag,$where);
		$term_id=I('request.term',7,'intval');
	...
	private function _listsc($where=array()){
		//sp_sql_posts_bycatid($cid,$tag,$where);
		$term_id=I('request.term',8,'intval');
	...
	private function _listse($where=array()){
		//sp_sql_posts_bycatid($cid,$tag,$where);
		$term_id=I('request.term',9,'intval');
	...
	private function _listsf($where=array()){
		//sp_sql_posts_bycatid($cid,$tag,$where);
		$shop = M("shop");
		$shop_relationships= M("shop_relationships");

		$term_id=I('request.term',10,'intval');

		// $where['post_type']=array(array('eq',1),array('exp','IS NULL'),'OR');

		if(!empty($term_id)){
		// 	$where['b.term_id']=$term_id;
			$term=$shop->where(array('term_id'=>$term_id))->find();
		 	$this->assign("term",$term);
		 }
		...
		if(!empty($term_id)){
			$shop->join("__TERM_RELATIONSHIPS__ b ON a.id = b.object_id");
		}
		...
		// 	->order("a.post_date DESC");
		if(empty($term_id)){
	 	$shop->field('a.*,c.user_login,c.user_nicename');
	 }else{
			$shop->field('a.*,c.user_login,c.user_nicename,b.listorder,b.tid');
			$shop->join("__TERM_RELATIONSHIPS__ b ON a.id = b.object_id");
		}
		 $posts=$shop->select();
	...
	private function _getTermTreed($term=array()){
		$result = $this->terms_model->order(array("listorder"=>"asc"))->select();
	...
	private function _getTermTreea($term=array()){
		$result = $this->terms_model->order(array("listorder"=>"asc"))->select();
	...
	private function _getTermTreeb($term=array()){
		$result = $this->terms_model->order(array("listorder"=>"asc"))->select();
	...
	private function _getTermTreec($term=array()){
		$result = $this->terms_model->order(array("listorder"=>"asc"))->select();
...
private function _getTermTreee($term=array()){
		$result = $this->terms_model->order(array("listorder"=>"asc"))->select();
	...
	private function _getTermTreef($term=array()){
		$shop=M("shop");
		$result = $shop->where(array("term_id"=>10))->select();
	...
	private function _getTermTreeg($term=array()){
		$result = $this->terms_model->order(array("listorder"=>"asc"))->select();
...
public function get_tree($myid, $str, $sid = 0, $adds = '', $str_group = '') {
	...
	private function _getTermTreeh($term=array()){
		$result = $this->terms_model->order(array("listorder"=>"asc"))->select();
